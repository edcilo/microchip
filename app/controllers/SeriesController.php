...
use microchip\series\SeriesUpdManager;
...
class SeriesController extends \BaseController {
	...
	public function show($ns, $id)
	{
		...
		if ( Request::ajax() ) return Response::json($series);
		...
		foreach($document->movements as $movement) {
            if ($movement->product->type == 'Product')
            {
                if ($movement->product->p_description->have_series) {
                    if ($type == 'sale') {
                        if ($movement->quantity != count($movement->seriesOut)) $result = 0;
                    } else {
                        if ($movement->quantity != count($movement->series)) $result = 0;
                    }
                }
            }
		}
